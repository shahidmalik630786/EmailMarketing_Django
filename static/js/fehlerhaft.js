...
$(document).ready(function () {
    ...
    $(document).on('click', '.btn-delete', function () {
        const fehlerhaftId = $(this).data('id');
        if (confirm("Are you sure you want to delete this record?")) {
            $.ajax({
                url: `/api/fehlerhaft/${fehlerhaftId}/`,
                method: 'DELETE',
                success: function (response) {
                    console.log("Data deleted successfully.");
                    fehlerhaftTable.ajax.reload();
                    getProjectID();
                },
    ...
    $('#insertFehlerhaft').on('click', function () {
        $(".error1").text('');
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#fehlerhaftTable').DataTable().ajax.reload(null, false);
...
function getFehlerhaft(id) {
    fehlerhaft_id = id
    fetch(`/api/fehlerhaft/data/${id}`, {
        method: 'GET',
        headers: {
...
function updateFehlerhaft() {
    ...
    $.ajax({
        url: `/api/fehlerhaft/${fehlerhaft_id}/`,
        method: 'PUT',
        contentType: 'application/json',
        data: JSON.stringify(data),
        success: function (response) {
            fehlerhaftTable.ajax.reload(null, false);
            $('input[type=text], input[type=email], input[type=password], input[type=number], textarea').val('');
            getProjectID();
            $('#closebutton').click();
        },
...
// Datatable
function loadList() {
    fehlerhaftTable = $('#fehlerhaftTable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "/api/fehlerhaft/",
        ...
        "columns": [
            {
                ...
                "render": function (data, type, row) {
                    return `
                    <button type="button" class="btn btn-warning mx-1 btnupdate" data-toggle="modal" data-target="#updateModal" data-id="${row.id}" onclick="getFehlerhaft('${row.id}')" ><i class="bi bi-pencil-square"></i></button>
